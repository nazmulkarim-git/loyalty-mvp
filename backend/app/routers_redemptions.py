...
def request_redeem(payload: RedeemRequestIn, current=Depends(get_current_user), db: Session = Depends(get_db)):
    e = db.query(models.Enrollment).filter(models.Enrollment.user_id==current.id, models.Enrollment.merchant_id==payload.merchant_id).first()
    if not e:
...
def pending_redemptions(merchant=Depends(require_role(models.RoleEnum.MERCHANT, models.RoleEnum.ADMIN)), db: Session = Depends(get_db)):
    ...
    if not m:
        return []
    rows = db.query(models.Redemption, models.Enrollment, models.Merchant) \             .join(models.Enrollment, models.Redemption.enrollment_id==models.Enrollment.id) \             .join(models.Merchant, models.Enrollment.merchant_id==models.Merchant.id) \             .filter(models.Redemption.status=="requested", models.Enrollment.merchant_id==m.id) \             .all()
    result = []
    for r,e,mm in rows:
        result.append(RedemptionOut(id=r.id, status=r.status, merchant_id=mm.id, merchant_name=mm.cafe_name, requested_at=r.created_at))
    return result
