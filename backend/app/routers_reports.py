...
def stamps_report(range: str = "day", merchant=Depends(require_role(models.RoleEnum.MERCHANT, models.RoleEnum.ADMIN)), db: Session = Depends(get_db)):
    ...
    count = db.query(models.Stamp).join(models.Enrollment, models.Stamp.enrollment_id==models.Enrollment.id) \            .filter(models.Enrollment.merchant_id==m.id, models.Stamp.created_at>=since).count()
    return {"count": count}
