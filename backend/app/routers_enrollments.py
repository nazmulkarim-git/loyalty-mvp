...
def my_enrollments(current=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for e in ens:
        ...
        result.append(EnrollmentOut(
            enrollment_id=e.id,
            merchant=MerchantOut(id=m.id, business_name=m.business_name, business_type=m.business_type, location_text=m.location_text, stamps_required=m.stamps_required, is_active=m.is_active),
